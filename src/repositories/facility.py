...
class RoomsFacilitiesRepository(BaseRepository):
    ...
    async def set_room_facilities(self, room_id: int, facility_ids: list[int]) -> None:
        get_current_facility_ids = (
            select(self.model.facility_id)
            .filter_by(room_id=room_id)
        )
        res = await self.session.execute(get_current_facility_ids)
